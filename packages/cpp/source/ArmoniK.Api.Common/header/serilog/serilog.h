...
*/
  std::string const logging_level_strings[6] = {
    "Verbose",
    "Debug",
    "Information",
    "Warning",
   ...
   *
   */
  std::string const logging_level_strings_short[6] = {
    "VRB",
    "DBG",
    "INF",
    "WRN",
   ...
   *
   */
    serilog_log_entry(std::string message,
                      serilog_context&& context)
    ...
    [[nodiscard]] std::string to_raw_json_entry() const
    {
      ...
      string_stream << R"({"@t": ")" << time << R"(", "@mt":")" << utils::string_tools::escape_json(_message) <<
        R"(", "@l":")"
        << logging_level_strings[context.level] << std::string(R"(","Logger":")") << context.logger_name <<
        std::string("\"");
      if (context.empty())
      {
        string_stream << "}";
      ...
      for (size_t i = 0; i < parameters_specified; ++i)
      {
        string_stream << ",\"" << utils::string_tools::escape_json(context[i].first) << "\":\"" <<
          utils::string_tools::escape_json(
            context[i].second.str_val) << "\"";
      }
      string_stream << "}";
 ...
 */
  ...
  public:
    ...
    */
    ...
    serilog& operator=(serilog const&) = delete;
    ...
    */
    ...
    void enrich(std::function<void(serilog_context&)> enrich)
    {
      enrich_.push_back(std::move(enrich));
    }
    ...
    */
    ...
    {
      instance_log_generic<logging_level::verbose>(std::move(message));
    }
    ...
    */
    ...
    {
      instance_log_generic<logging_level::debug>(std::move(message));
    }
    ...
    */
    ...
    {
      instance_log_generic<logging_level::info>(std::move(message));
    }
    ...
    */
    ...
    {
      instance_log_generic<logging_level::warning>(std::move(message));
    }
    ...
    */
    ...
    {
      instance_log_generic<logging_level::error>(std::move(message));
    }
    ...
    */
    ...
    {
      instance_log_generic<logging_level::fatal>(std::move(message));
    }
  ...
  private:
    ...
    {
      if (L < level && L < level_serilog) return;
     ...
     */
    template <logging_level L>
    void instance_log_generic(std::string message) const
    {
      if (L < level && L < level_serilog) return;
