...
mod tests {
    ...
    fn a_certificate_variable_names_a_path_without_reading_it() {
        // Whether the paths lead anywhere is `connect`'s question to ask, not this one's: nothing
        // here touches a filesystem, which is what lets this test name files that do not exist.
        let config = with_var(
        ...
        .expect("reading the variables themselves must not fail");
