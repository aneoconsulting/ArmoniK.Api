...
pub use armonik_transport::{
    ConfigError, ConnectionError, EnvFieldError, Http2Config, HttpConfig, IdentitySource,
    ProxyConfig, ProxyError, ProxySource, SecretString, TcpConfig, TlsConfig,
};
...
#[cfg(feature = "_gen-client")]
#[doc(hidden)]
pub use env::{ConfigSnafu, ConnectSnafu, EnvSnafu};
...
impl Client<tonic::transport::Channel> {
    ...
    pub async fn new() -> Result<Self, NewClientError> {
        let config = HttpConfig::from_env(ARMONIK_PREFIX).context(EnvSnafu)?;
    ...
    async fn get_nb_request(service: &str, rpc: &str) -> usize {
        ...
        let mut config = HttpConfig::from_env(ARMONIK_PREFIX).unwrap();
