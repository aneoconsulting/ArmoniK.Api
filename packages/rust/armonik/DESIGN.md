...
| Risk | Mitigation |
|---|---|
| `prost::encoding` is `#[doc(hidden)]` and documented as "only for prost internal use", so it is not covered by semver at all | pinned prost 0.14; the surface is small and enumerable (`encode_key`, `decode_key`, `encode_varint`, `encoded_len_varint`, `key_len`, `skip_field`, `check_wire_type`, `merge_loop`, `WireType`, and the `int32`/`string`/`message`/`hash_map` submodules), it is the same surface prost-derive expands to, and breakage is a compile error rather than a wire bug. `DecodeError::new`, the one item already deprecated there, is unused |
...
| Convenience methods | Hand-written in `client/*.rs`, each claimed by `#[armonik(rpc = "...")]` so the macro can check every RPC has one and harvest its docs. Generating them from the request's fields was built and reverted: see section 5.8 |
...
| Client | One `.call()` for all three RPC kinds (section 3.5, section 5.17). The convenience methods stay hand-written in `client/*.rs`, tied back to the rpc lines by `#[armonik_macros::client]`; section 5.8 records why generating them cost lines |
...
# Part IV: the uniformity pass

Part III removed duplication. This part records the rule that removed the
*structure* producing it, because the rule generalizes and the individual cuts do
not.

## 12.1 The preference

- **A degenerate case falls out of the general code; it does not get its own
  path.** An empty list interpolates to nothing, so the shape carrying none of
  something is the shape carrying some of it, with a shorter list. Where a branch
  exists only because one arm has zero of what another has, the branch is the
  defect.
- **Two orthogonal axes are not a product of named cases.** Where a sum type
  carries the coordinates it already encodes, or a mode is inferred from a flag
  plus an emptiness test, the fact has more than one home and the homes can
  disagree.
- **A shared helper over a closed set of cases should be total.** Where
  `emit::slot_write` covered all four `SlotCodec`s and `slot_merge_in_place`
  declared two of them unreachable, those two had grown a bespoke emitter each.
  The hole on the read side was the whole reason there were three variant
  emitters instead of one.

Two corollaries, because both were nearly got wrong here:
...
- **Prefer removing a restriction to adding a guard.** A non-oneof field whose
  ...
  it. Ordering each arm's own fields made the shape emit correctly, and the check
  unnecessary.
...
`Normalize` fragment, so a shared field under a `with` adapter would have lost
its equivalence classes. Collected once per field now.

## 12.3 Considered and rejected

Four explorations of the design space. Each entry is refuted by a fact; refute
the fact before re-proposing.

- **Inverting the source of truth**, so the descriptor generates the Rust surface
  instead of validating it. The resolution machinery is paid for by *deviations*,
  not by direction: 149 of 185 type names differ from the proto leaf name, 12
  proto messages are claimed by 36 Rust types, and there are 377 deviation
  declarations. A side table expressive enough is isomorphic to `objects/` and
  has no rustc checking, no rustdoc and no spans. Compile time is not a motive
  either: prost-build over the same 40 protos is 2.04 s against this crate's
  2.33 s.
- **Adopting an existing crate.** No Rust mechanism holds the combination. Each
  capability exists somewhere -- femtopb's `EnumValue<E>` is the dataful open
  enum, bilrost has bare-enum oneofs, micropb's `Config::custom_field` is
  `ProtoAdapter<T>` -- but compile-time validation of a hand-written type against
  an external schema exists in exactly one place in the ecosystem, cxx's extern
  enums. `autoproto` set out to build it for protobuf and wrote it into its
  README as future work. prost issues #276, #437, #764 and #1079 request the
  three headline capabilities and have sat 4-6 years; the maintainer has stated
  they are not taking features.
- **Unifying `ProtoField` and `ProtoAdapter` into one `Codec<T>`.** 2% of
  `codec/`, paid for with a coherence experiment and hand-written call sites that
  no longer infer `Self` from their arguments. `ProtoAdapter` exists so two
  codecs can target one Rust type (`HashMap` as a proto map, and as
  `repeated MapResultTask`); that needs a separate `Self` and nothing else does.
- **A table-driven encoder** instead of emitted per-field statements. `dyn BufMut`
