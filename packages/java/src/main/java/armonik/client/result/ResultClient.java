...
import static armonik.api.grpc.v1.results.ResultsFilters.Filters;


/**
 * ResultClient provides methods for interacting with result-related functionalities.
 * It communicates with a gRPC server using a blocking stub to perform various operations on results.
 */
public class ResultClient {
  ...
  /**
   * Deletes result data for the specified session ID and result IDs.
   *
   * @param sessionId the session ID associated with the results to delete
   * @param resultIds the list of result IDs to delete
   * @return a DeleteResultsDataResponseRecord containing the deleted result IDs
   */
  ...
  /**
   * Creates metadata for results associated with the specified session ID and names.
   *
  ...
  /**
   * Retrieves a map of result IDs to task IDs for the specified session ID and result IDs.
   *
  ...
  /**
   * Lists results based on the specified filters, total count, pagination parameters, and sorting criteria.
   *
