...
public interface IResultClientAsync {
  ...
  StreamObserver<ResultsCommon.WatchResultRequest> watchResults(StreamObserver<ResultsCommon.WatchResultResponse> responseObserver);

  void getServiceConfiguration(StreamObserver<Integer> responseObserver);

  void deleteResultsData(String sessionId, List<String> resultIds, StreamObserver<SessionDeletedResultIds> responseObserver);

  void downloadResultData(String sessionId, String resultId, StreamObserver<byte[]> responseObserver);

  StreamObserver<ResultsCommon.UploadResultDataRequest> uploadResultData(String sessionId, String resultId, String payload, StreamObserver<?> responseObserver);
