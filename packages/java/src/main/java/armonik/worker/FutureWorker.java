...
public class FutureWorker extends WorkerGrpc.WorkerImplBase {
    ...
    /**
     * Constructs a new FutureWorker with the specified managed channel and task
     * handler consumer.
     *
     * @param channel             the managed channel for communication with the
     *                            Agent server
     * @param taskHandlerConsumer the consumer function to handle FutureTaskHandler
     *                            for task processing
     */
    ...
    public void process(ProcessRequest request, StreamObserver<ProcessReply> responseObserver) {
        try {
            ...
            System.out.println(taskHandler.getPayloadId());
