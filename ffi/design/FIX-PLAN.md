...
| R-H25 | CPU definition (req 21): the "or" lets an RPC cell count one thread; codec CPU is thread CPU in four slices and one process-wide value per case (GC forced by BDN included, no per-round CPU) in C# | CS4, CP2, MV4 || owner 2026-09-26: process CPU, per round, codec and RPC (CAMPAIGN req 21) |
| R-H26 | Content sets: SHAPES.md does not say which payloads carry them; the slices share no (payload, non-ASCII set) pair (Python P2.4 only, Rust and C# P1.2 and P2.2, C++ five payloads, Java all) | CP1, MV8 || owner 2026-09-26: P1.2, P2.2 and P2.4 in every slice (req 7) |
| R-H27 | U-* rows differ: C++ has no encode direction; Python and Java use the corpus-schema core; Java has no `accept` filter; counts 92 or 311 | CP4 || owner 2026-09-26: the 92 accepted rows at the shapes core's 7 ABI roots, 3 directions, timed shapes core (req 7); rows at other shapes messages or corpus roots are labelled extras |
| R-H28 | RPC transport: UDS in C# and Java, loopback TCP elsewhere; Java's "shipped" has no source in `packages/java`; C# passes `adaptive_window = 0` in shipped | CP7 || owner 2026-09-26: UDS in every slice; Java shipped = grpc-java defaults, stated (req 17) |
| R-H29 | Timing scope (req 11): graph construction and the encode end state are not fixed per arm (C++ incumbent allocates a ByteBuffer and core-ffi reuses; Python core-ffi returns fresh `bytes`; Java encodes a new graph per iteration from a 32 MiB pool, the others one hot graph) | CS7, CP9 || owner 2026-09-26: measure all variants: end state (reused buffer; transport-ready) and input (hot graph; beyond-cache pool) (req 11) |
| R-H30 | Req 16 fixes blocking delivery for B and C only; A and D may be async | CS5 || owner 2026-09-26: idiomatic per stack, stated (req 16) |
| R-H31 | Req 19: the gate does not count resets, runs on a counting build rather than the timed one, does not cover RPC cells, and leaves retain buffer sizing (so grow crossings) to the harness | CS6 || owner 2026-09-26: count resets; RPC cells per call; retain counts with no pre-placed buffer and exact grow (req 19). Timed-binary check not required |
| R-H32 | GC and JIT state between blocks, warm-up placement, JIT tier with no consequence (req 24, 25) | CS8 || owner 2026-09-26: framework defaults, stated (req 24) |
| R-H33 | Server identity per launch, server warm-up and connection lifetime unspecified (req 13, 17) | CS9 || owner 2026-09-26: one server per launch for every cell and both builds, warmed; one channel per cell per launch (req 13) |
| R-H34 | Idle states, SMT inside CLIENT, and CLIENT size not fixed for the campaign (req 2, 4) | CS10 || owner 2026-09-26: set sizes fixed (4 and 4, ffi/campaign.machine); worker thread counts in headers (req 4). Reference machine i9-7900X; idle states and SMT not fixed |
| R-H35 | No cell serves option 2 (host-gen codec over the core's transport), though README section 13 says option 2 depends on B-A with the generated codec | CS || owner 2026-09-26: add cells E (host-gen, core transport) and F (host-gen, host gRPC) (req 12) |
| R-H36 | RPC direction (a): Python's like-for-like row is `a+read` (upb is lazy); a mapping is needed | CP10 || owner 2026-09-26: a and a+read in every slice; cross-slice uses a+read (req 14) |
