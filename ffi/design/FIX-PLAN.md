...
| R-G11 | Unknown fields inside an inlined singular child, a oneof message member or a map entry have no decode-side slot in the C ABI, so retain mode writes the dropped form there (16 rows; rust D34) | rust slice, WP5 | **mechanism specified by the owner 2026-09-24**: per-message `ak_unk_buf` slots in the decode groups, configured by a generated per-root options struct (one entry per message position) at context creation and reset; replaces the decode callbacks (ABI-v1 decision 11). Implementation: WP5 step 7 |
