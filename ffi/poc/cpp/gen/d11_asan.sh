#!/usr/bin/env bash
# WP5 step 9 (ABI v1 decision 11): the binding's unknown-field buffers under AddressSanitizer
# and LeakSanitizer. Every buffer the binding hands the core is malloc'd and is freed on
...
# Log: ffi/logs/cpp/wp5s9-asan.log
set -u
cd "$(dirname "$0")/.." || exit 2
L=../../logs/cpp
KEEP=${1:-}
...
mkdir -p "$A"
{
  echo "# cpp slice, WP5 step 9: decision 11 under ASan + LSan"
...
} > "$L/wp5s9-asan.log" 2>&1
[ -z "$KEEP" ] && rm -rf "$A"
grep -q ">>> FAIL" "$L/wp5s9-asan.log" && exit 1 || exit 0
