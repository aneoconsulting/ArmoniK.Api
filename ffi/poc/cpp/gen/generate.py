...
def corpus_targets():
    ...
    return {
        ...
        **corpus_nounk_targets(full, abi, refused),
    }


def nounk(p):
    """WP5 step 10: the same plan relowered with unknown="drop", THE NO-UNKNOWN VARIANT
    (plan.py); every omission is the plan's, this glue only selects it."""
    return P.relower(p, p.options.with_unknown("drop"))


def corpus_nounk_targets(full, abi, refused):
    an = nounk(abi)
    hdr, lay, names = c_abi.emit(an)
    return {
        "corpus/nounk/include/ak_abi.h": hdr,
        "corpus/nounk/include/generated/ak_layout.h": lay,
        "corpus/nounk/include/generated/ak_layout_names.h": names,
        "corpus/src/generated/binding_nounk.h": cpp_binding.emit_header(
...
def emit_dispatch(full, abi, refused, retain=None, native_retain=True):
    ...
    sn = cpp_names.snake
    ffi_retain = FFI_RETAIN if retain is None else retain
    ...
    for name in (abi.roots if ffi_retain else []):
...
def targets():
    ...
    out = {
        ...
        **_borrow(p),
        # WP5 step 10: THE NO-UNKNOWN VARIANT (unknown fields compiled out): its own header
        # (AK_NO_UNKNOWN_FIELDS, 240 layout facts) in a variant include dir, and the binding
        # rendered from the drop plan. Selected per build by include path and macro.
        **_nounk_targets(p),
...
def _nounk_targets(p):
    pn = nounk(p)
    hdr, lay, names = c_abi.emit(pn)
    out = {
        "nounk/include/ak_abi.h": hdr,
        "nounk/include/generated/ak_layout.h": lay,
    ...
    try:
        ...
        out["src/generated/binding_borrow_nounk.cpp"] = cpp_binding.emit(
            pn, ns="shapes_borrow", hdr="generated/binding_borrow_nounk.h")
    finally:
