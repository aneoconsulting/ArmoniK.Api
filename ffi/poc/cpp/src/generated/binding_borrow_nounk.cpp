...
struct ak_efix_Probe make_probe(const Probe &o, const Tcs &t) {
  ...
  g.body_as_int = o.body.which() == shapes_borrow::ProbeBody::kAsInt ? o.body.as_int() : 0;
  g.body_as_text = o.body.which() == shapes_borrow::ProbeBody::kAsText ? ak_str_of(o.body.as_text(), t.utf8) : ak_str_absent();
  g.body_as_blob = o.body.which() == shapes_borrow::ProbeBody::kAsBlob ? ak_str_of(o.body.as_blob(), t.bytes) : ak_str_absent();
  g.body_as_stamp = o.body.which() == shapes_borrow::ProbeBody::kAsStamp ? make_timestamp(o.body.as_stamp(), t) : ak_efix_Timestamp();
  g.body_as_nothing = o.body.which() == shapes_borrow::ProbeBody::kAsNothing ? make_empty(o.body.as_nothing(), t) : ak_efix_Empty();
...
static int32_t loop_list_tasks_detailed_response_tasks_parent_task_ids(ak_enc_ctx *ctx, const void *obj, int64_t token) {
  AK_GUARD_BEGIN
    ...
    const std::vector<ak::StringView> &src = src_owner.parent_task_ids;
...
static int32_t loop_list_tasks_detailed_response_tasks_data_dependencies(ak_enc_ctx *ctx, const void *obj, int64_t token) {
  AK_GUARD_BEGIN
    ...
    const TaskDetailed &el = (*h->o).tasks[(size_t)token];
    const TaskDetailed &src_owner = el;
    const std::vector<ak::StringView> &src = src_owner.data_dependencies;
    constexpr size_t kChunk = ak::arena_n(sizeof(struct ak_str));
    struct ak_str chunk[kChunk];
    size_t i = 0, done = 0;
    (void)done;
...
static int32_t loop_list_tasks_detailed_response_tasks_expected_output_ids(ak_enc_ctx *ctx, const void *obj, int64_t token) {
  AK_GUARD_BEGIN
    ...
    const TaskDetailed &el = (*h->o).tasks[(size_t)token];
    const TaskDetailed &src_owner = el;
    const std::vector<ak::StringView> &src = src_owner.expected_output_ids;
    constexpr size_t kChunk = ak::arena_n(sizeof(struct ak_str));
    struct ak_str chunk[kChunk];
    size_t i = 0, done = 0;
    (void)done;
...
static int32_t loop_list_tasks_detailed_response_tasks_retry_of_ids(ak_enc_ctx *ctx, const void *obj, int64_t token) {
  AK_GUARD_BEGIN
    ...
    const TaskDetailed &el = (*h->o).tasks[(size_t)token];
    const TaskDetailed &src_owner = el;
    const std::vector<ak::StringView> &src = src_owner.retry_of_ids;
    constexpr size_t kChunk = ak::arena_n(sizeof(struct ak_str));
    struct ak_str chunk[kChunk];
    size_t i = 0, done = 0;
    (void)done;
...
static int32_t loop_list_tasks_detailed_response_tasks_options_options(ak_enc_ctx *ctx, const void *obj, int64_t token) {
  AK_GUARD_BEGIN
    ...
    const std::map<ak::StringView, ak::StringView> &src = src_owner.options;
    ...
    size_t i = 0, done = 0;
    (void)done;
    for (std::map<ak::StringView, ak::StringView>::const_iterator it = src.begin(); it != src.end(); ++it) {
...
intptr_t encode_into_upload_result_data_message(ak_enc_ctx *ctx, const UploadResultDataMessage &o, const Tcs &t) {
  ...
  AK_TAX();
  const ak::StringView &dbuf = (*o.upload).data_chunk;
...
static void add_list_tasks_detailed_response_tasks_options_options(ak_dec_ctx *ctx, void *obj, int64_t tok, const struct ak_dfix_TaskOptionsOptionsEntry *elems, int32_t n) {
  AK_DGUARD_BEGIN
    Sink_ListTasksDetailedResponse *s = (Sink_ListTasksDetailedResponse *)obj;
    std::map<ak::StringView, ak::StringView> &dst = s->out->tasks[(size_t)tok].options.get_or_insert().options;
    ak::StringView k_, v_;
    for (int32_t i = 0; i < n; ++i) {
