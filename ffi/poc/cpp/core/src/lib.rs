//! The core, behind the C ABI. **cpp slice copy.**
//!
//! Byte-for-byte the rust slice's `crates/ak-core/src/lib.rs` apart from this note, the
//! `rpc` feature gate and nothing else: it is hand-written runtime support, not a codec,
//! so R1 is satisfied by `generated/codec.rs` being emitted by the SAME emitter the rust
//! slice uses (`ffi/poc/rust/gen/rust_abi.py`, imported read-only by `gen/generate.py`).
//! This crate is built BOTH as a cdylib (the primary arm) and as a staticlib (the second,
//! separately labelled arm).
//!
