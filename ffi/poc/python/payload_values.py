"""The payload builder (harness glue, not generated): facade objects carrying EXACTLY the
manifest's values.

Was `gen/out/payload_values.py`, "emitted" by `gen/py_values.py` -- which returned this text
verbatim and read nothing from the description, so it was never generated output. It moved
here when FIX-PLAN WP5 step 5 retired the slice's own generator backends. It builds facade
objects through constructors passed in; it contains no wire rule.
