...
from plan import (FIXED, abi_order_topo, as_plan, dec_vtable, direct_fields, element_types,
                  ...
                  unk_entry_points, unk_opts_layout, unknown_compiled_out)
...
def emit(x, extra_asserts=()):
    ...
    for root in ([] if nounk else p.roots):
        ...
        o.append(" * pool (rule 1); `host` is passed to every grow as `sink`. */")
        o.append("struct %s {" % on)
        o.append("  void *host;")
        for mn, m, ty in unk_opts_layout(p, root):
            o.append("  struct %s %s;  /* %s */" % (ty, mn, m))
        o.append("};")
