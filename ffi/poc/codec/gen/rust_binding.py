...
from plan import (as_plan, direct_fields, elem_type, loop_slots, presence_bits,  # noqa: F401
                  slot_elem as slot_elem_rust, slot_name)
...
def emit_binding(ir):
    ...
    for root in ir.roots:
        ...
        for si, (path, f) in enumerate(loop_slots(ir, root)):
            if elem_type(f) and f.card != "map":
                # Through the Options of any inlined parent. A map slot has no entry to
                # attribute to: a facade map entry carries no bag (U-map-entry, disputed).
                head, expr = [], "out"
                for i, p in enumerate(path[:-1]):
                    head.append("if let Some(c%d) = %s.%s.as_mut() {" % (i, expr, p))
                    expr = "c%d" % i
                inner = ("if let Some(e) = %s.%s.get_mut(token as usize) { e.unknown_fields.extend_from_slice(&bytes); }"
                         % (expr, path[-1]))
                o.append("            %d => { %s%s%s }" % (si + 1, " ".join(head), inner, " }" * len(head)))
        o.append("            _ => {}")
        o.append("        }")
        o.append("    }")
        o.append("    Ok(out)")
