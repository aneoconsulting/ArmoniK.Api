...
def _emit_loop_unk_zeroed(ir, o, root, path, f, sn):
    ...
    o.append("        // Decision 9 as corrected: clear the elements you will fill, min(n, CHUNK), not")
    o.append("        // the whole arena (O(elements), not O(arena)).")
    o.append("        let total = src.len();")
    o.append("        ::core::ptr::write_bytes(chunk.as_mut_ptr() as *mut u8, 0, total.min(CHUNK) * SZ);")
    ...
    o.append("                done += i;")
    o.append("                ::core::ptr::write_bytes(chunk.as_mut_ptr() as *mut u8, 0, (total - done).min(CHUNK) * SZ);")
    o.append("                i = 0;")
    o.append("            }")
    o.append("        }")
    o.append("        if i > 0 {")
...
def _emit_loop_zeroed(ir, o, root, path, f, sn):
    ...
    o.append("                done += i;")
    o.append("                // Only what the next chunk will fill is put back.")
    o.append("                ::core::ptr::write_bytes(chunk.as_mut_ptr() as *mut u8, 0, (total - done).min(CHUNK) * SZ);")
    o.append("                i = 0;")
    o.append("            }")
    o.append("        }")
