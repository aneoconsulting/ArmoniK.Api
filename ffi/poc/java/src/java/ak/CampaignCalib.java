 ...
 * and the same core the arms use (`ak_noop`), timed per round on the process CPU clock (req 21 as amended) and
...
public final class CampaignCalib {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int r = 1; r <= Campaign.ROUNDS; r++) {
      long c0 = Campaign.processCpuNs(), t0 = System.nanoTime();
      long s = 0;
      for (int i = 0; i < n; i++) s += Native.noop(i);
      long t1 = System.nanoTime(), c1 = Campaign.processCpuNs();
      sink += s;
