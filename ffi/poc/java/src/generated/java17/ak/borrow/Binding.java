...
public final class Binding implements AutoCloseable, ak.Callbacks {
  ...
  /** The batching predicate of ABI v1 section 6, as a flag rather than a verdict. The cpp
   *  slice measured the crossover at a forward crossing of roughly 2 to 4 ns and this
   *  host's is about 11, so the prediction is that batching wins here; it is measured as a
   *  delta between the two arms in the same interleaved rounds (R4), not as two ratios. */
  ...
  // to make the call, so an offset is one add and then the same fused transcode. Indexing
  // the managed array instead measures 14 %% worse, and it is the obvious thing to write."
  ...
  void fillUploadResultData(long g, UploadResultData o) {
    ...
    putBytes(g + ak.shapes.Layout.AK_EFIX_UPLOADRESULTDATA__DATA_CHUNK, o.data_chunk);
   ...
   *  same copy. Indexing the managed array instead measured 14 percent
   *  worse, and it is the obvious thing to write. */
  ...
  long dec3() {   // ListTasksDetailedResponse.new tasks
    ...
    ((ListTasksDetailedResponse) decRoot).tasks.add(x);
  ...
  long dec13() {   // ListTaskSummaryResponse.new tasks
    ...
    ((ListTaskSummaryResponse) decRoot).tasks.add(x);
    return pushDecToken(x);
  }
  ...
  long dec18() {   // ListMetricsResponse.new batches
    ...
    ((ListMetricsResponse) decRoot).batches.add(x);
    return pushDecToken(x);
  }
