...
# directory, which is this slice's own and is gitignored.
set -eu
cd "$(dirname "$0")/.."
HERE=$PWD
RUST=$PWD/../rust
...
(cd "$RUST" && AK_BENCH_ONLY=P1.1 "$CARGO_TARGET_DIR/release/bench" 2>&1) \
  | grep -E 'crossing|^#|linkage|cdylib' | head -40
echo
echo "# and this slice's own, in the same shape: a JNI forward call and a JNI upcall,"
echo "# measured in one process against a no-op on both sides of the boundary."
J17=${J17:-/usr/lib/jvm/java-17-openjdk-amd64}
unset JAVA_TOOL_OPTIONS || true
"$J17/bin/java" --add-opens java.base/java.nio=ALL-UNNAMED -cp build/probe \
  -Dak.lib="$PWD/build/probe/libprobe.so" Probe 2>&1 | tail -9
echo
echo "# and the core's own no-op entry point, reached the way every arm reaches it, so"
echo "# the crossing price is measured through the SAME shim the arms use."
CP=$(cat deps/cp.txt)
"$J17/bin/java" -cp "build/cls17:$CP" -Dak.lib="$PWD/build/jni/libakjni.so" ak.RunNoop 2>&1
