"""Arm `core-ffi`: the Java half of the binding.
...
* **Batching.** ABI v1 section 6 says a host may decline. The cpp slice measured the
  crossover at a forward crossing of roughly 2 to 4 ns and JNI's is 11; the `batch` flag
  switches the two arms inside one process so the question is a delta in interleaved rounds
  (R4's sharpened form) and not two ratios to a third arm.
...
# The offsets live in ONE class. The borrowed facade of decision 13 is a second package
# over the same ABI, and re-emitting the layout table there would be a second thing to
# get right for no gain: a borrowed view changes the FACADE, not the group.
...
def _off(sname, member):
    return "%s.%s" % (LAYOUT[0], L.group_const(sname, member))


def _size(sname):
    return "%s.%s" % (LAYOUT[0], L.group_const(sname))


def _group_members(ir, m, enc):
    """(field-or-None, member name, member type) in group order, the same walk
    `rust_abi.group_fields` makes, but carrying the IR field so the fill knows what to
    write. `None` marks the oneof discriminant."""
    out = []
    for f in m.plain:
        if f.oneof or f.card != "singular":
            continue
        out.append((f, f.name, None))
    for oname, members in m.oneofs.items():
        out.append((None, "%s_case" % oname, oname))
        for g in members:
            out.append((g, "%s_%s" % (oname, g.name), oname))
    return out
...
def _fill_one(ir, o, ind, f, sname, member, src, bits, owner, sparse,
    ...
    if sparse:
        zero = N.ZERO[f.kind]
        o.append("%sif (%s != %s) " % (p, src, zero) if f.kind != "bool"
                 else "%sif (%s) " % (p, src))
        _fill_scalar(o, ind + 2, f, sname, member, src)
    else:
...
def _emit_apply(ir, o, name):
    ...
    o.append("   *  same copy. Indexing the managed array instead measured 14 percent")
    o.append("   *  worse, and it is the obvious thing to write. */")
...
def _run_call(et, leaf, entry):
    if leaf:
        return "ak.NativeEntry.elem%s(ctx, chunkp, i)" % et
    return "ak.NativeEntry.elemu%s(ctx, chunkp, i, tok0)" % et
...
def _emit_dec_slot(ir, o, owner, kind, sn, fld, slot):
    ...
    if kind == "new":
        ...
        o.append("    ((%s) decRoot).%s.add(x);" % (owner, sn))
...
def emit(ir, level=17, ns=N.PKG, facade_ns=None):
    ...
    vt_addr = {}
    total = 0
    order = RA.vtable_messages(ir)
    for name in order:
        rows = S.enc_vtable(ir, name)
        vt_addr[("e", name)] = total
        total += len(rows)
    for name in ir.roots:
        rows = S.dec_vtable(ir, name)
        vt_addr[("d", name)] = total
        total += len(rows)
    o.append("    vt = Mem.alloc(%dL * 8);" % total)
    for name in order:
        base = vt_addr[("e", name)]
        for j, (kind, sn, et) in enumerate(S.enc_vtable(ir, name)):
            at = base + j
            if kind == "loop":
                o.append("    Mem.U.putLong(vt + %d * 8, ak.NativeEntry.encTrampoline(%d));   // %s.loop %s"
                         % (at, enc_ix[(name, sn)], name, sn))
            elif kind == "elem":
    ...
    for name in ir.roots:
        base = vt_addr[("d", name)]
        for j, (kind, sn, et) in enumerate(S.dec_vtable(ir, name)):
            at = base + j
            if kind in ("unknown", "unk"):
                o.append("    Mem.U.putLong(vt + %d * 8, 0L);   // %s.%s %s: decision 11's"
                         " bag is not built" % (at, name, kind, sn))
            else:
                o.append("    Mem.U.putLong(vt + %d * 8, ak.NativeEntry.decTrampoline(%d));   // %s.%s %s"
                         % (at, dec_ix[(name, kind, sn)], name, kind, sn))
    for name in order:
    ...
    for root in ir.roots:
        ...
        if direct:
            ...
            o.append("    byte[] direct = %s;" % _null_guard(expr))
            o.append("    long rc = ak.NativeEntry.encodeDirect%s(this, encCtx, evt%s, g,"
                     " direct, direct.length);" % (root, root))
        else:
        ...
        o.append("    decRoot = r;")
        o.append("    int rc = ak.NativeEntry.decode%s(this, decCtx, wireNative, len,"
                 " dvt%s);" % (root, root))
        o.append("    check(rc);")
        o.append("    return r;")
        o.append("  }")

    PULL.emit(ir, dec_ix, o)
