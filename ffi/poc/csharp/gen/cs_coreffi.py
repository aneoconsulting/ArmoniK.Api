...
def emit(ir):
    ...
    o += "    public int Count;"
    o += "    /// Elements per ak_elem_* call. 0 means the whole run in one call."
    o += "    /// The Rust slice's host chunks at 150, which is the ENTIRE reason its"
    o += "    /// crossing counts differ from this slice's; see JOURNAL.md entry 19."
    o += "    public int Chunk;"
    o += "}"
    o += ""
    ...
    o += "    /// Elements per ak_elem_* call; 0 hands the whole run over at once."
    o += "    public int Chunk;"
    ...
    o += "                int rc = Abi.ak_elem_ResultRaw(ctx, run->Groups + off, n);"
    o += "                if (rc < 0) return rc;"
    o += "            }"
    o += "            return 0;"
    o += "        }"
    ...
    o.doc("The host-side half on its own: zero the by-value group and stage every "
          "string, then stop without calling the codec. The difference between this and "
          "`Encode` is the codec plus every crossing, MEASURED rather than argued. It "
          "exists because the M1 absent path's 2.361 was attributed to the group fill "
          "by elimination, and elimination is not a measurement.", "    ")
    o += "    public int Fill(ListResultsResponse src) => FillOrEncode(src, false, out _, out _);"
    o += ""
    o += "    public int Encode(ListResultsResponse src, out byte* outPtr, out int outLen)"
    o += "        => FillOrEncode(src, true, out outPtr, out outLen);"
    o += ""
    o += "    private int FillOrEncode(ListResultsResponse src, bool call, out byte* outPtr, out int outLen)"
    o += "    {"
    ...
    o += "        ForwardCalls += Chunk <= 0 ? 1 : (n + Chunk - 1) / Chunk;   // ak_elem_* per chunk"
    ...
    o += "    }"
    o += ""
    o.doc("The CORE's counters for the last operation, in the core's own convention. "
          "Zero from a non-counting build.", "    ")
    o += "    public AkCounters EncCounters()"
    o += "    {"
    o += "        AkCounters c; Abi.ak_enc_counters(_ctx, &c); return c;"
    o += "    }"
    o += ""
    o += "    public void EncCountersReset() => Abi.ak_enc_counters_reset(_ctx);"
    o += ""
    o += "    public AkCounters DecCounters()"
    o += "    {"
    o += "        AkCounters c;"
    o += "        if (_dctx == IntPtr.Zero) return default;"
    o += "        Abi.ak_dec_counters(_dctx, &c); return c;"
    o += "    }"
    o += ""
