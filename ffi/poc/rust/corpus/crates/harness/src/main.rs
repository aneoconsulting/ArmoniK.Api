...
fn parent(manifest: &Path, timeout: Duration, only: &[String]) -> i32 {
    ...
    if total_fail == 0 && hard == 0 {
        println!("CORPUS PASSES on all {} arms", ARMS.len());
        0
    } else {
...
fn main() {
    ...
    if unkc {
        ...
        {
            let _ = plant;
            eprintln!("--unk-controls: this is the no-unknown build (unknown-field support compiled out)");
            std::process::exit(2);
        }
    }
...
// Decision 11's controls (WP5 steps 7-8) live in `unkctl.rs`: they exist only in the build
