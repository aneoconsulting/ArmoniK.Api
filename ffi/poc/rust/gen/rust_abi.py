...
"""
from ir import loop_slots, slot_name
            ...
            if f.card == "singular" and f.kind == "message":
                ...
                if et and not ir.msg(et).leaf:
                    ...
                    rd = {"double": "%s.f64()", "bool": "(%s.varint() != 0) as u8",
                          "int32": "%s.varint() as i32", "int64": "%s.varint() as i64"}[f.kind]
                    o.append("            %d if wire == 2 => {" % f.tag)
...
def _emit_add(ir, o, root, et, sn, path, f):
    ...
    if f.card == "map":
        ...
        conv = "!= 0" if f.kind == "bool" else ""
        o.append("        dst.reserve(n as usize);")
        o.append("        for i in 0..n as usize { dst.push(*elems.add(i) %s); }" % conv)
    elif f.kind == "message":
        o.append("        dst.reserve(n as usize);")
