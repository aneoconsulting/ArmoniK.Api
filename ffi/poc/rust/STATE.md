...
generate.py      writes every core file (shapes core, corpus core, both no-unknown variants),
                 then runs every slice's generator; --check adds drift + the guard over the 26
                 backend modules with its planted violation
...
`ffi-retain`, `native-drop`, `native-retain`; no-unknown build `ffi-nounk`, `native-drop`,
`native-retain`. RPC cells A, B, C and D (C and D in retain, drop and no-unknown).
...
each binary loads the core of its variant. Smoke run of the current harness:
`logs/rust/campaign-wp5s10/` (codec, rpc; gate); calib smoke: `logs/rust/campaign/`.
...
| 32 | smoke run | met: `logs/rust/campaign-wp5s10/` (gate; codec both builds; rpc both clients x both transports, plant control aborting on each) and `logs/rust/campaign/` (calib), figures marked instrumentation in every header |
