...
| | |
|---|---|
| **Status** | **stages 1 to 6 complete, plus FIX-PLAN WP4 item 1 (finding R-D1).** The length-varint wrap in the shared core was reproduced on the current code (all three claims plus the R-D9 u32 item, `logs/rust/rd1-wrap-unfixed.log`), fixed in `poc/codec/` with the aggregating session's authorization (behaviour change to `ak-rt`'s `len_body`/`f64` and to the decode emitter in `gen/rust_abi.py`), and the full correctness gate re-run unchanged (byte identity 16/16, pull value identity 16/16, unknown-field vectors, four-build concurrency suite). See D22 and D23 below. Everything before that: **stages 1 to 6 complete.** Stages 1 to 4 as before (four arms, every shape, the RPC arm, decision 3 on both sides). **Stage 5** adds ABI v1 section 7.1's **pull decode family** (open decision 2, both halves), obligation 12.5's **concurrency suite**, and section 3's **`ak_init` and lifecycle**. **Stage 6** prices section 9's **three deliveries at the floor** and builds the **A/B/C RPC grid**, and came back with two things nobody asked for: **the codec is ~60% of a real RPC's CPU**, and **there is no loopback-TCP penalty — R9's flow-control hazard is a Nagle artifact in the harness**. All of it landed in the SHARED core at `poc/codec/` (R0), additively |
| **Blocked on** | nothing |
...
gen/rd1_repro.sh [label]    FIX-PLAN WP4 item 1 / R-D1: each wrap case in its own
                            process under `timeout 5`, C ABI and core-native. Builds
                            `rdrepro`. Run once with "unfixed", once with "fixed"
...
**`contentall`** (the content sets on every payload), **`rdrepro`** (FIX-PLAN WP4
item 1 / R-D1: one length-wrap case per process, C ABI and core-native).
...
| # | Where | What | Status |
|---|---|---|---|
| D22 | `poc/codec/crates/ak-rt/src/dec.rs` `len_body`, and `gen/rust_abi.py`'s decode emitter | **The length-varint wrap (FIX-PLAN WP4 item 1, finding R-D1), reproduced then fixed.** In release builds `overflow-checks` is off, so `self.pos + n > self.buf.len()` in `len_body` wrapped for a length varint near 2^64 and the bounds check passed. Three consequences, all reproduced on the unfixed code (`logs/rust/rd1-wrap-unfixed.log`): **(a)** an unknown field whose skip length wraps moves `pos` backwards and the root loop re-reads it forever — the finding's literal 11 bytes `7A F5 FF FF FF FF FF FF FF FF 01` to `ak_decode_ListResultsResponse` HANGS (timeout, both the C-ABI and the core-native path); **(b)** a string field's wrapping length passes the check and yields a span with `len` 0x`FFFFFFFF`, and the trailing `flush!()`/`apply` after the loop delivered it — the observing vtable saw `session_id.len=4294967295` with `apply_called=true` and `rc=-2`, so a host would read out of bounds; **(c)** a nested-message wrapping length made `&buf0[off..off + n]` **panic inside `extern "C"` and abort the process** (SIGABRT, "panic in a function that cannot unwind"). | **fixed and authorized (WP4 item 1, aggregating session, behaviour change to `poc/codec/`)**: `len_body` and `f64` now compare against the REMAINING bytes (`n > self.buf.len() - self.pos`, with `pos <= len` established), which cannot overflow and returns an empty in-bounds span on rejection — so every `&buf[off..off + n]` on the decode path is sliceable by construction and cannot panic. The decode emitter now guards the trailing `flush!()`/`apply` (push) and the `OP_APPLY`/`OP_APPLY_ELEM` record (pull) on `d.err == 0`, so **nothing is delivered after a decode error**. Swept across the shared emitter, not hand-edited; `gen/generate.py --check` clean in this slice and the codec generator. After the fix every case returns promptly (`logs/rust/rd1-wrap-fixed.log`): (a) `rc=-3` no hang, (b) `rc=-3` no bad span and `apply_called=false`, (c) `rc=-3` no abort. ak-rt carries three `len_body` unit regressions. **Byte-identity gate unchanged** (conformance 16/16, pull value-identity 16/16), because on valid input `d.err == 0` |
| D23 | `gen/rust_abi.py` decode + parse entry points | **`u32` truncation of span offsets/lengths for a buffer over 4 GiB (R-D9 minor item).** Spans are `(u32, u32)` into the input buffer; a buffer longer than `u32::MAX` would alias offsets and lengths. On the unfixed code the entry points had no length check, so a length above `u32::MAX` was accepted and the reader ran off the buffer (`logs/rust/rd1-wrap-unfixed.log`, `u32-ffi`: timeout). | **fixed** as part of WP4 item 1: `ak_decode_*` and `ak_parse_*` reject `len > u32::MAX as usize` at entry with `AK_ERR_LIMIT` before any access. `logs/rust/rd1-wrap-fixed.log` `u32-ffi`: `rc=-5`, a clean reject. Core-native materialises owned values (no spans), so it is not affected |
...
|---|---|---|
| `ffi/logs/rust/rd1-wrap-unfixed.log` | rustc 1.94.1 release (overflow-checks off), cdylib boundary; each case `timeout 5`, one process per case | **R-D1 reproduced on the UNFIXED core.** (a) the finding's 11 bytes hang both the C-ABI and core-native decode (exit 124); (b) a wrapping string length is delivered as a span with `len` 0x`FFFFFFFF` and `apply_called=true` after `rc=-2`; (c) a wrapping nested-message length aborts the process across `extern "C"` (SIGABRT); u32: a `>u32::MAX` length is accepted and the reader runs off the buffer (hang). The baseline the fix is measured against |
| `ffi/logs/rust/rd1-wrap-fixed.log` | as above, after the fix | **The same nine cases all return promptly.** (a) `rc=-3` no hang; (b) `rc=-3`, no bad span, `apply_called=false`; (c) `rc=-3`, no abort; u32 `rc=-5` (`AK_ERR_LIMIT`) clean reject. The gate FIX-PLAN WP4 item 1 sets |
| `ffi/logs/rust/rd1-conformance.log` | rustc 1.94.1 release, cdylib, guard on, ASCII | Byte identity re-run after the core change: every arm byte-identical to the validated manifest on every payload, decode round trip ok. The manifest-hash gate, unchanged by the fix |
| `ffi/logs/rust/rd1-shapes.log` | as above | Explicit presence, the oneof, and the seven unknown-field vectors, all arms agree — unchanged by the fix |
| `ffi/logs/rust/rd1-concurrency.log` | rustc 1.94.1 release, four builds (shipped, global-widths, pad-widths, both) in separate target dirs | Obligation 12.5's suite re-run: the two must-PASS builds clean and the two planted must-FAIL builds caught, unchanged by the fix |
