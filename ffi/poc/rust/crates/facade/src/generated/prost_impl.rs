// @generated by ffi/poc/codec/gen/rust_facade.py (plan from ffi/schema/shapes.json). Do not edit.
...
impl ::prost::Message for Probe {
    fn encode_raw(&self, buf: &mut impl ::prost::bytes::BufMut) {
        ...
        if let Some(ProbeBody::AsInt(x)) = &self.body { ::prost::encoding::int64::encode(10, x, buf); }
        if let Some(ProbeBody::AsText(x)) = &self.body { ::prost::encoding::string::encode(11, x, buf); }
        if let Some(ProbeBody::AsBlob(x)) = &self.body { ::prost::encoding::bytes::encode(12, x, buf); }
        if let Some(ProbeBody::AsStamp(x)) = &self.body { ::prost::encoding::message::encode(13, x, buf); }
        if let Some(ProbeBody::AsNothing(x)) = &self.body { ::prost::encoding::message::encode(14, x, buf); }
    }
    fn encoded_len(&self) -> usize {
        let mut n = 0usize;
        if !self.id.is_empty() { n += ::prost::encoding::string::encoded_len(1, &self.id); }
        ...
        if let Some(ProbeBody::AsInt(x)) = &self.body { n += ::prost::encoding::int64::encoded_len(10, x); }
        if let Some(ProbeBody::AsText(x)) = &self.body { n += ::prost::encoding::string::encoded_len(11, x); }
        if let Some(ProbeBody::AsBlob(x)) = &self.body { n += ::prost::encoding::bytes::encoded_len(12, x); }
        if let Some(ProbeBody::AsStamp(x)) = &self.body { n += ::prost::encoding::message::encoded_len(13, x); }
        if let Some(ProbeBody::AsNothing(x)) = &self.body { n += ::prost::encoding::message::encoded_len(14, x); }
        n
    }
    fn merge_field(
        &mut self,
