...
| W8 | **Conformance corpus.** Section 10. **In flight**, on its own branch and its own machine, owning `corpus/` and nothing else. Two decisions were settled before it started: **the corpus is a superset and need not be expressible in `SHAPES.md`** (item 5 needs a shape the schema has no room for, and obliging five built slices to add a facade message is not worth it this late), and **a vector carries a set of accepted encodings rather than one hash**, because P2.5 proved two forms can both be right. | Every slice produces and consumes the same bytes, and the corpus is generated rather than curated. |
